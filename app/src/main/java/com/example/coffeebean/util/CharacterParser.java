...
public class CharacterParser {
    ...
    public static ArrayList<ContactInfo> search(String str,
                                                ArrayList<ContactInfo> allContacts, ArrayList<ContactInfo> contactList) {
        contactList.clear();
        // 如果搜索条件以0 1 +开头则按号码搜索
        ...
        for (ContactInfo contact : allContacts) {
            ...
            result = finder.getSelling(str);
            if (contains(contact, result)) {
                contactList.add(contact);
            } else if (contact.getPhoneNumber().contains(str)) {
    ...
    public static boolean contains(ContactInfo contact, String search) {
        ...
        if (search.length() < 6) {
            String firstLetters = contact.getLetter();
            // 不区分大小写
        ...
        if (!flag) { // 如果简拼已经找到了，就不使用全拼了
            ...
            Matcher matcher2 = pattern2.matcher(finder.getSelling(contact.getNoteName()));
            flag = matcher2.find();
            //部分字符全拼匹配
            if(!flag) {
                search=search.toLowerCase();
                String NN=finder.getSelling(contact.getNoteName()).toLowerCase();
                if(NN.indexOf(search)!=-1){//找到
                    flag=true;
                };
            }
        }
