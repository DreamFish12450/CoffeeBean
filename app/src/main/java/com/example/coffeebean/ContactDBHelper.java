...
//        sqLiteDatabase.execSQL(ContactInfo.CREATE_TABLE);
     ...
     */
    public long insertContactInfo(ContactInfo ContactInfo, Context context) {
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues values = new ContentValues();
        int id1 = 0;
        if (context != null){
             id1 = UserManage.getInstance().getUserInfo(context).getId();
        }
        values.put(ContactInfo.COLUMN_ID,id1);
    ...
    public ArrayList<ContactInfo> getContactInfoContactId(int Id) {
     ...
     * @return 读取数据库，返回一个 ContactInfo 类型的 ArrayList
     */
    public ContactInfo getContactInfoBynoteName(String CnoteName) {
        ContactInfo ContactInfo = new ContactInfo();
        SQLiteDatabase db = this.getReadableDatabase();
        String[] columnArray = new String[]{
                ContactInfo.COLUMN_ID,
