...
import static com.example.coffeebean.adapter.ContactInfoAdapter.*;
        ...
        public boolean onTouchEvent(MotionEvent e) {
            ...
            switch (e.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    ...
                    if (item != null) {
                        //获取当前选中的Item
                        RecyclerHolder recyclerHolder = (RecyclerHolder) getChildViewHolder(item);
//                        Contact.ViewHolder viewHolder = (MyAdapter.ViewHolder) getChildViewHolder(item);
                        mCurItemLayout = recyclerHolder.itemView.findViewById(R.id.contact_info_item_linear);

                        //找到具体元素（这与实际业务相关了~~）
                        mLlHidden = (LinearLayout) recyclerHolder.itemView.findViewById(R.id.ll_hidden);
                        mItemDelete = (LinearLayout) recyclerHolder.itemView.findViewById(R.id.ll_hidden);
                        mItemDelete.setOnClickListener(new View.OnClickListener() {
                            @Override
                            public void onClick(View v) {
                                if (mRightListener != null) {
                                    //删除
                                    ContactInfoAdapter contactInfoAdapter = new ContactInfoAdapter((RecyclerView) v);
                                    contactInfoAdapter.removeData(curSelectPosition);
